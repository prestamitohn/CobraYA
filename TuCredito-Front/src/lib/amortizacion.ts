// Motor de amortización de CobraYA — fuente única de verdad.
//
// Portado desde TuCredito-API/Services/Implementations/CalculadoraService.cs, con dos
// cambios deliberados frente al original:
//  1. El sistema de amortización se identifica por nombre (no por id numérico), porque
//     en el .NET original los ids sembrados en la tabla SistAmortizacion no coincidían
//     con el switch del calculador (bug: id 1 "Francés" en realidad ejecutaba la fórmula
//     Directa). Con un enum de string ese desfase no puede volver a ocurrir.
//  2. Las fechas de vencimiento dependen de frecuenciaCobro (diario/semanal/quincenal/
//     mensual) en vez de asumir siempre +k meses — clave para el mercado hondureño,
//     donde el cobro diario/semanal es predominante.
//
// ⚠️ Existe una copia idéntica en supabase/functions/_shared/amortizacion.ts para que
// las Edge Functions (Deno) puedan usarla sin depender de imports fuera de su árbol.
// Este archivo es el original — cualquier cambio a las fórmulas debe replicarse ahí.
